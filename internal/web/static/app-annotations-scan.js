...
function buildImportedScanAnnotation(entry, index, payload, deps) {
  ...
  const mappedRects = rects.length > 0 ? rects : Number.isFinite(line) && line > 0 ? approximateTextRectsForLine(line, clamp01) : Number.isFinite(paragraph) && paragraph > 0 ? approximateTextRectsForParagraph(paragraph, clamp01) : [];
  return {
    ...base,
    type: "highlight",
    target: "text",
...
function createScanAnnotationController(deps) {
  const {
    clamp01,
    collectNormalizedClientRects,
    createAnnotationID,
  ...
  async function confirmImportedScanAnnotations(selected) {
    const pending = Array.isArray(selected) ? selected.filter((entry) => safeText(entry?.source) === "scan_upload" && Number(entry?.scan_artifact_id || 0) > 0) : [];
...
export {
  createScanAnnotationController
};

//# sourceMappingURL=app-annotations-scan.js.map
