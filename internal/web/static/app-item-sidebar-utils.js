...
function normalizeItemSidebarFilters(rawFilters = null) {
  ...
  const workspaceUnassigned = String(workspaceRaw || "").trim().toLowerCase() === "null" || filters.workspace_unassigned === true;
...
function itemSidebarEndpoint(view, filters = state.itemSidebarFilters) {
...
function itemSidebarCountsEndpoint(filters = state.itemSidebarFilters) {
  const normalizedFilters = normalizeItemSidebarFilters(filters);
...
function setInboxTriggerCount(count) {
  ...
  const normalizedCount = Number.isFinite(Number(count)) && Number(count) > 0 ? Math.trunc(Number(count)) : 0;
...
function isGitHubPRSidebarItem(item) {
...
function defaultItemSidebarLaterVisibleAfter(now = /* @__PURE__ */ new Date()) {
...
async function fetchItemSidebarWorkspaces() {
...
async function fetchItemSidebarProjects() {
  ...
  })).filter((project) => project.id && project.name && project.id !== "hub" && (!project.sphere || project.sphere === normalizeActiveSphere(state.activeSphere)));
}
async function performItemSidebarSphereUpdate(item, nextSphere) {
...
async function performItemSidebarReviewDispatch(item, target, value = "") {
  const itemID = Number(item?.id || 0);
...
async function performItemSidebarWorkspaceUpdate(item, workspaceID = null, workspaceName = "") {
  const itemID = Number(item?.id || 0);
...
async function performItemSidebarProjectUpdate(item, projectID = "", projectName = "") {
  const itemID = Number(item?.id || 0);
  if (itemID <= 0) return false;
...
async function performItemSidebarTriage(item, action, options = {}) {
  ...
  if (normalizedAction === "later") {
    body.visible_after = defaultItemSidebarLaterVisibleAfter(options.now || /* @__PURE__ */ new Date());
...
async function showItemSidebarWorkspaceMenu(item, x, y) {
  try {
...
async function showItemSidebarProjectMenu(item, x, y) {
  try {
...
function showItemSidebarReviewMenu(item, x, y) {
...
export {
  applyItemSidebarCounts,
  defaultItemSidebarCounts,
  defaultItemSidebarLaterVisibleAfter,
  fetchItemSidebarActors,
  fetchItemSidebarProjects,
  fetchItemSidebarWorkspaces,
  hideItemSidebarMenu,
  isEmailSidebarItem,
  isGitHubPRSidebarItem,
  itemSidebarActionLabel,
  itemSidebarCountsEndpoint,
  itemSidebarEndpoint,
  itemSidebarGestureAction,
  itemSidebarMenuEl,
  itemSidebarStatusText,
  maybeShowSomedayReviewNudge,
  normalizeItemSidebarCounts,
  normalizeItemSidebarFilters,
  normalizeItemSidebarView,
  performItemSidebarProjectUpdate,
  performItemSidebarReviewDispatch,
  performItemSidebarSphereUpdate,
  performItemSidebarStateUpdate,
  performItemSidebarTriage,
  performItemSidebarWorkspaceUpdate,
  positionItemSidebarMenu,
  refreshItemSidebarCounts,
  setInboxTriggerCount,
  showItemSidebarActionMenu,
  showItemSidebarDelegateMenu,
  showItemSidebarMenu,
  showItemSidebarProjectMenu,
  showItemSidebarReviewMenu,
  showItemSidebarWorkspaceMenu
};

//# sourceMappingURL=app-item-sidebar-utils.js.map
