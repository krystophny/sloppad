...
async function openCompanionWorkspaceView(viewKind, filePath) {
  ...
  const endpoint = viewKind === "transcript" || viewKind === "summary" || viewKind === "references" ? viewKind : "";
...
function isCurrentMeetingSummaryView(projectID, filePath) {
  return String(state.activeProjectId || "").trim() === String(projectID || "").trim() && normalizeWorkspaceBrowserPath(state.workspaceOpenFilePath) === normalizeWorkspaceBrowserPath(filePath);
}
...
async function submitMeetingSummaryItems(projectID, panel) {
  ...
  const selected = Array.from(panel.querySelectorAll('.meeting-summary-items-list input[type="checkbox"]:checked')).map((node) => Number(node.value || "-1")).filter((value) => Number.isInteger(value) && value >= 0);
  ...
  try {
    ...
    if (status instanceof HTMLElement) {
      status.textContent = createdCount === 1 ? "1 inbox item created from this summary." : `${createdCount} inbox items created from this summary.`;
    }
...
function isLikelyPrReviewArtifact(payload) {
  ...
  return /(?:^|\/)\.tabura\/artifacts\/pr\/pr-\d+\.(?:diff|patch)$/.test(title) || /(?:^|\/)artifacts\/pr\/pr-\d+\.(?:diff|patch)$/.test(title);
}
export {
  clearMeetingSummaryItemsPanel,
  companionViewKindForPath,
  countSelectedMeetingSummaryItems,
  exitPrReviewMode,
  isCurrentMeetingSummaryView,
  isLikelyPrReviewArtifact,
  isMobileViewport,
  loadWorkspaceBrowserPath,
  maybeEnterPrReviewModeFromTextArtifact,
  meetingSummaryItemsButtonLabel,
  normalizeWorkspaceBrowserPath,
  openCompanionWorkspaceView,
  openWorkspaceSidebarFile,
  parentWorkspaceBrowserPath,
  parseUnifiedDiffFiles,
  refreshWorkspaceBrowser,
  renderActivePrReviewFile,
  renderMeetingSummaryItems,
  renderPrReviewFileList,
  resetPrReviewUi,
  resolveWorkspaceSteppingCurrentFile,
  setFileSidebarAvailability,
  setMeetingSummaryItemsBusy,
  setPrReviewActiveFile,
  setPrReviewDrawerOpen,
  sidebarFileKindForPath,
  statusBadgeForDiffFile,
  stepCanvasFile,
  stepPrReviewFile,
  stepWorkspaceFile,
  submitMeetingSummaryItems,
  updateMeetingSummaryItemsSelection,
  workspaceCompanionEntries,
  workspaceNavigableFilePaths
};

//# sourceMappingURL=app-pr-review.js.map
