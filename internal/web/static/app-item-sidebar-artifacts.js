...
function appendIdeaPromotions(detail, promotions) {
  ...
  records.forEach((entry) => {
    ...
    const refsList = Array.isArray(entry?.refs) ? entry.refs.map((ref) => String(ref || "").trim()).filter(Boolean) : [];
...
function buildIdeaNoteMarkdown(title, artifactMeta) {
  ...
  const notes = Array.isArray(artifactMeta?.notes) ? artifactMeta.notes.map((entry) => String(entry || "").trim()).filter(Boolean) : [];
...
function sidebarEmailBody(meta = {}) {
  return String(
    meta.body || meta.body_text || meta.text || meta.summary || meta.content || meta.snippet || ""
  ).trim();
}
...
function buildEmailThreadHTML(title, artifactMeta) {
  const subject = String(artifactMeta?.subject || title || "Email thread").trim() || "Email thread";
  const messages = Array.isArray(artifactMeta?.messages) ? artifactMeta.messages : [];
  if (messages.length === 0) {
    return null;
...
function buildSidebarItemFallbackText(item, artifact = null) {
  ...
  const body = String(
    artifactMeta.transcript || artifactMeta.text || artifactMeta.body || artifactMeta.summary || artifactMeta.content || ""
  ).trim();
  if (body) {
...
export {
  appendIdeaPromotionPreview,
  appendIdeaPromotions,
  buildEmailThreadHTML,
  buildIdeaNoteMarkdown,
  buildSidebarItemFallbackText,
  ideaRefinementHeading,
  openSidebarArtifactItem,
  parseSidebarArtifactMeta
};

//# sourceMappingURL=app-item-sidebar-artifacts.js.map
