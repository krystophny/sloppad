...
const VOICE_VAD_NO_SPEECH_MS = 4e3;
const VOICE_VAD_MAX_RECORDING_HARD_MS = 24e4;
const HOTWORD_VAD_NO_SPEECH_MS = 7e3;
...
function newMediaRecorder(stream) {
  ...
  const isSupported = typeof window.MediaRecorder?.isTypeSupported === "function" ? (t) => window.MediaRecorder.isTypeSupported(t) : () => false;
...
function canUseMicrophoneCapture() {
  return Boolean(window.MediaRecorder) && Boolean(navigator.mediaDevices) && typeof navigator.mediaDevices.getUserMedia === "function";
}
...
function stopChatVoiceMedia(capture) {
  ...
  if (capture._sileroDeferred) {
    try {
      capture._sileroDeferred.destroy();
    } catch (_) {
    }
    capture._sileroDeferred = null;
  }
  if (capture._vadStream) {
    for (const track of capture._vadStream.getTracks()) {
      track.stop();
    }
  ...
  if (capture._vadAudioContext) {
    try {
      capture._vadAudioContext.close();
    } catch (_) {
    }
...
async function startSileroVADMonitor(capture) {
  ...
  try {
    ...
    const instance = await initVAD({
      stream: vadStream,
      audioContext: capture._vadAudioContext || void 0,
      ...
      onSpeechEnd(audio) {
        ...
        if (audio instanceof Float32Array && audio.length > 0) {
          capture._vadAudioBlob = float32ToWav(audio, 16e3);
        }
        stopVADMonitor(capture);
        void stopVoiceCaptureAndSend();
      }
  ...
  try {
    ...
    if (capture._vadAudioBlob) {
      ...
      if (capture.chunks.length > 0) {
        sttBlob = mimeType ? new Blob(capture.chunks, { type: mimeType }) : new Blob(capture.chunks);
        if (!mimeType) {
...
export {
  ...
  handleSTTWSMessage,
  handleVADNoSpeechTimeout,
  invalidateCachedMicStream,
  isLikelyIOS,
  isVoiceVADAutoSendEnabled,
  newMediaRecorder,
  observeCachedMicStream,
  openPrintView,
  parseOptionalBoolean,
  recordHarnessPrintAction,
  recordHarnessSTTAction,
  releaseMicStream,
  requestMicRefresh,
  startSileroVADMonitor,
  startVADMonitor,
  stopChatVoiceMedia,
  stopChatVoiceMediaAndFlush,
  stopVADMonitor,
  stopVoiceCaptureAndSend,
  streamHasLiveAudioTrack,
  sttCancel,
  sttSendBlob,
  sttStart,
  sttStop,
  voiceCaptureEmptyReasonMessage
};

//# sourceMappingURL=app-voice.js.map
