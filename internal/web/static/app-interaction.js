...
function normalizeInteractionTool(modeRaw) {
...
function normalizeInteractionSurface(modeRaw) {
...
function interactionConversationMode({
  surface = state.interaction.surface,
  tool = state.interaction.tool
} = {}) {
  if (state.liveSessionActive && (state.liveSessionMode === "dialogue" || state.liveSessionMode === "meeting")) {
...
function interactionSurfaceDefaultForPane(paneId) {
  if (paneId !== "canvas-text") return "annotate";
  if (shouldAnnotateTextArtifactByDefault()) return "annotate";
  return "editor";
}
function applyInteractionDefaultsForPane(paneId) {
...
function setInteractionSurface(surface) {
...
function renderInteractionSurfaceToggle() {
...
async function selectInteractionTool(tool) {
...
function renderToolPalette() {
  ...
  for (const mode of TOOL_PALETTE_MODES) {
    ...
    button.addEventListener("click", () => {
      updateRuntimePreferences({ tool: mode.id }).then(() => {
        if (mode.id !== "ink") {
          clearInkDraft();
        }
        renderInkControls();
...
function maybeApplySelectionHighlight() {
...
export {
  applyInteractionDefaultsForPane,
  canToggleInteractionSurface,
  currentCanvasPaneId,
  initToolPalette,
  interactionConversationMode,
  interactionSurfaceDefaultForPane,
  isInkTool,
  ...
  setInteractionSurface
};

//# sourceMappingURL=app-interaction.js.map
