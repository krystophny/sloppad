...
function showCanvasColumn(paneId) {
  ...
  const artifactKind = paneId === "canvas-image" ? "image_artifact" : paneId === "canvas-pdf" ? "pdf_artifact" : "text_artifact";
  ...
  state.currentCanvasArtifact = {
    kind: artifactKind,
    title: artifactTitle,
    surfaceDefault: currentArtifact.kind === artifactKind && currentArtifact.title === artifactTitle ? String(currentArtifact.surfaceDefault || "") : ""
  };
...
function hasLocalAssistantWork() {
  return state.pendingQueue.length > 0 || state.pendingByTurn.size > 0 || state.assistantActiveTurns.size > 0 || state.assistantUnknownTurns > 0;
}
function hasRemoteAssistantWork() {
  return state.assistantRemoteActiveCount > 0 || state.assistantRemoteQueuedCount > 0;
}
function hasLocalStopCapableWork() {
  return state.assistantActiveTurns.size > 0 || state.assistantUnknownTurns > 0 || state.assistantCancelInFlight;
}
...
function isDirectAssistantWorking() {
  return hasLocalStopCapableWork() || state.assistantRemoteActiveCount > 0 || state.assistantRemoteQueuedCount > 0;
}
...
function setVoiceLifecycle(next, reason = "") {
  const normalized = Object.values(VOICE_LIFECYCLE).includes(next) ? next : VOICE_LIFECYCLE.IDLE;
...
function isStopCapableLifecycle(mode = state.voiceLifecycle) {
  return mode === VOICE_LIFECYCLE.LISTENING || mode === VOICE_LIFECYCLE.STOPPING_RECORDING || mode === VOICE_LIFECYCLE.AWAITING_TURN || mode === VOICE_LIFECYCLE.ASSISTANT_WORKING;
}
...
function isUiStopGestureActive() {
  return shouldStopInUiClick() || isVoiceTranscriptSubmitPending() || state.voiceTranscriptSubmitInFlight || hasPendingOverlayTurn();
}
...
function isTemporaryCanvasArtifactTitle(title) {
  const normalized = String(title || "").trim().replaceAll("\\", "/").replace(/^\.\//, "").toLowerCase();
  return normalized.startsWith(".tabura/artifacts/tmp/") || normalized.startsWith("tabura/artifacts/tmp/");
}
...
export {
  ...
  canStartLiveDialogueListen,
  chatHistoryEl,
  currentIndicatorMode,
  deriveVoiceLifecycle,
  hasLocalAssistantWork,
  hasLocalStopCapableWork,
  hasPendingOverlayTurn,
  hasRemoteAssistantWork,
  hideCanvasColumn,
  isAssistantWorking,
  isDirectAssistantWorking,
  isRealCanvasArtifactEvent,
  isStopCapableLifecycle,
  isTTSSpeaking,
  isTemporaryCanvasArtifactTitle,
  isUiReadyForStatus,
  isUiStopGestureActive,
  ...
  shouldStopInUiClick,
  showCanvasColumn,
  startVoiceLifecycleOp,
  syncChatScroll,
  syncVoiceLifecycle,
  updateAssistantActivityIndicator
};

//# sourceMappingURL=app-canvas-ui.js.map
