...
async function openItemSidebarView(view = state.itemSidebarView, filters = null) {
...
function stepItemSidebarItem(delta) {
...
async function loadItemSidebarView(view = state.itemSidebarView, filters = null) {
...
async function openSidebarItem(item) {
  ...
  if (String(item?.artifact_kind || "").trim().toLowerCase() !== "github_pr") {
    try {
      const opened2 = await openSidebarArtifactItem(item);
      if (opened2 && isMobileViewport()) {
        closeEdgePanels();
      }
    } catch (err) {
...
function formatSidebarAge(value) {
  ...
  const seconds = Math.max(0, Math.floor((Date.now() - parsed) / 1e3));
...
function renderWorkspaceFileList(list) {
  ...
  rows.forEach((entry) => {
    ...
    list.appendChild(renderSidebarRow({
      ...
      label: entryName,
      active: activeWorkspaceSelectionPath ? entryPath === activeWorkspaceSelectionPath : !isDir && activeWorkspaceFilePath && entryPath === activeWorkspaceFilePath,
...
export {
  activeItemSidebarShortcutTarget,
  buildItemSidebarBadges,
  buildItemSidebarSubtitle,
  formatSidebarAge,
  handleItemSidebarKeyboardShortcut,
  itemIconForRow,
  itemKindLabel,
  itemSourceLabel,
  loadItemSidebarView,
  ...
  openSidebarItem,
  openSidebarPRReview,
  parsePullRequestNumberFromSourceRef,
  parseSidebarTimestamp,
  renderItemSidebarList,
  renderSidebarRow,
  renderSidebarTabs,
...
//# sourceMappingURL=app-item-sidebar-ui.js.map
