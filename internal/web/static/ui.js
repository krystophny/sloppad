...
function showIndicatorMode(mode, x, y) {
  ...
  if (body) {
    const isCueVisible = nextMode === "recording" || nextMode === "play" || nextMode === "listening" || nextMode === "paused" || nextMode === "cursor";
...
function showTextInput(x, y, anchor) {
  const el = inputEl();
  if (!el) return;
...
function updateOverlay(markdownText) {
  const content = overlayContentEl();
  if (!content) return;
...
function getInputAnchor() {
  return uiState.inputAnchor;
}
function setInputAnchor(anchor) {
  uiState.inputAnchor = anchor || null;
}
function pinCursorAnchor(x, y, anchor) {
  uiState.cursorPinned = true;
  uiState.cursorAnchor = anchor || null;
  uiState.inputAnchor = anchor || null;
  setLastInputPosition(x, y);
}
function clearCursorAnchor() {
...
function buildContextPrefix(anchor) {
  if (!anchor) return "";
  ...
  const hasPoint = Number.isFinite(normalizedX) && Number.isFinite(normalizedY) && normalizedX >= 0 && normalizedX <= 1 && normalizedY >= 0 && normalizedY <= 1;
  ...
  const pointLabel = hasPoint ? `${Math.round(normalizedX * 100)}%, ${Math.round(normalizedY * 100)}%` : "";
  if (hasPage && hasLine) {
