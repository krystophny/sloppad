...
async function createNewMailDraft() {
  try {
...
async function replyToSidebarItem(item) {
...
function mailAuthoringUsesDictation() {
  return String(state.interaction?.tool || "").trim().toLowerCase() === "prompt" && String(state.interaction?.surface || "").trim().toLowerCase() === "annotate";
}
function mailReplyArtifactTitle(item) {
  return String(item?.artifact_title || item?.title || "").trim();
}
async function launchNewMailAuthoring() {
...
async function launchReplyAuthoring(item) {
  if (!mailAuthoringUsesDictation()) {
...
async function replyAllToSidebarItem(item) {
  const itemID = Number(item?.id || 0);
  if (itemID <= 0) return false;
  try {
...
async function launchReplyAllAuthoring(item) {
  if (!mailAuthoringUsesDictation()) {
...
async function forwardSidebarItem(item) {
  const itemID = Number(item?.id || 0);
  if (itemID <= 0) return false;
  try {
...
async function launchForwardAuthoring(item) {
  if (!mailAuthoringUsesDictation()) {
...
async function openMailDraftArtifact(artifactID) {
...
async function sendActiveMailDraft() {
  const artifactID = Number(state.mailDraft?.artifactId || 0);
...
function renderMailDraftArtifact(root, event) {
...
export {
  ...
  openMailDraftArtifact,
  renderMailDraftArtifact,
  replyAllToSidebarItem,
  replyToSidebarItem,
  resetMailDraftState,
  sendActiveMailDraft
};

//# sourceMappingURL=app-mail-drafts.js.map
