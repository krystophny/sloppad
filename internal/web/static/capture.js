  ...
  (function() {
    ...
    function setCaptureState(nextState) {
      ...
      if (cleanState === "failed") {
        ...
        recordHint.textContent = state.fallbackVisible ? "Retry the memo or save it as text instead." : "Retry the memo or clear to discard.";
        return;
      }
      recordLabel.textContent = "Record";
      if (state.audioBlob) {
        recordHint.textContent = state.pendingTranscript ? "Memo captured. Retry save or clear to discard." : "Memo captured. Retry transcription or clear to discard.";
        return;
      }
      ...
      try {
        ...
        if (isNetworkFailure(error)) {
          holdVoiceMemoInMemory(
            state.pendingTranscript ? "Offline. The transcript is kept in memory until you reconnect." : "Offline. Voice memo kept in memory until you reconnect."
          );
    ...
    function offerTextFallback() {
      ...
      noteInput.focus();
      setCaptureState("failed");
      setStatus(transcript ? "Transcript copied into the note field. Edit it if needed, then save as text." : "Type the memo while it is fresh, then save it as a text note.", "");
      updateSaveState();
    }
