...
function getEdgeTapSizePx() {
  ...
  try {
    return window.matchMedia(EDGE_TAP_SIZE_SMALL_MEDIA_QUERY).matches ? EDGE_TAP_SIZE_SMALL_PX : EDGE_TAP_SIZE_PX;
  } catch (_) {
    return EDGE_TAP_SIZE_PX;
  }
}
function getTopEdgeTapSizePx() {
  ...
  try {
    return window.matchMedia(EDGE_TAP_SIZE_SMALL_MEDIA_QUERY).matches ? EDGE_TOP_TAP_SIZE_SMALL_PX : EDGE_TOP_TAP_SIZE_PX;
  } catch (_) {
    return EDGE_TOP_TAP_SIZE_PX;
  }
}
...
function toggleTopEdgeDrawer(edgeTop) {
...
function initEdgePanels() {
  ...
  document.addEventListener("mousemove", (ev) => {
    ...
    if (edgeTop && !edgeTop.classList.contains("edge-pinned")) {
      const inTopTrigger = ev.clientY < topEdgeTapSize && !topEdgeTriggerBlockedByFileSidebar();
    ...
    if (inputRow) {
      ...
      window.addEventListener("orientationchange", () => {
        baselineHeight = Math.max(
          window.innerHeight,
          window.visualViewport ? window.visualViewport.height + Math.max(0, window.visualViewport.offsetTop || 0) : window.innerHeight
        );
...
export {
  ...
  handleRasaEdgeTap,
  initEdgePanels,
  isLeftEdgeTapCoordinate,
  toggleFileSidebarFromEdge,
  toggleRightEdgeDrawer,
  toggleTopEdgeDrawer
};

//# sourceMappingURL=app-edge-panels.js.map
