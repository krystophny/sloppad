...
function syncInteractionBodyState() {
...
function strokeBounds(strokes) {
  const points = Array.isArray(strokes) ? strokes.flatMap((stroke) => Array.isArray(stroke?.points) ? stroke.points : []) : [];
...
function mapInkPointToClient(point) {
  ...
  if (state.inkDraft.target === "pdf" && state.inkDraft.pageInner instanceof HTMLElement) {
    const rect2 = state.inkDraft.pageInner.getBoundingClientRect();
    return {
      x: rect2.left + (Number(point.x) || 0),
      y: rect2.top + (Number(point.y) || 0)
    };
  }
  const viewport = document.getElementById("canvas-viewport");
...
function buildInkEventPayload() {
  ...
  const rawLocations = sampledPoints.map((point) => mapInkPointToClient(point)).filter(Boolean).map((clientPoint) => getLocationFromPoint(clientPoint.x, clientPoint.y)).filter((location) => location && typeof location === "object");
  const lineNumbers = rawLocations.map((location) => Number(location?.line || 0)).filter((line) => Number.isFinite(line) && line > 0);
  ...
  const width = state.inkDraft.target === "pdf" ? Math.max(1, Number(state.inkDraft.pageWidth) || 1) : Math.max(1, Number(inkLayerEl()?.viewBox.baseVal?.width || inkLayerEl()?.getAttribute("width") || 1));
  const height = state.inkDraft.target === "pdf" ? Math.max(1, Number(state.inkDraft.pageHeight) || 1) : Math.max(1, Number(inkLayerEl()?.viewBox.baseVal?.height || inkLayerEl()?.getAttribute("height") || 1));
  const snapshotDataURL = state.inkDraft.target === "pdf" ? buildPdfInkSnapshotDataURL() : `data:image/png;base64,${buildInkPNGBase64()}`;
  const overlappingLines = lineNumbers.length > 0 ? { start: Math.min(...lineNumbers), end: Math.max(...lineNumbers) } : null;
  return {
    type: "canvas_ink",
    cursor: cursor ? {
      line: Number(cursor.line || 0) || void 0,
      page: Number(cursor.page || 0) || void 0,
      title: String(cursor.title || ""),
      surrounding_text: String(cursor.surroundingText || ""),
      relative_x: Number.isFinite(cursor.relativeX) ? cursor.relativeX : void 0,
      relative_y: Number.isFinite(cursor.relativeY) ? cursor.relativeY : void 0
    } : null,
...
function beginInkStroke(pointerEvent) {
  const pdfAnchor = activeArtifactKindForInk() === "pdf" ? pdfPageAnchorAtPoint(pointerEvent.clientX, pointerEvent.clientY) : null;
  if (pdfAnchor) {
    ...
    path2.setAttribute("stroke-width", stroke2.width.toFixed(2));
    appendInkPointToPath(path2, stroke2);
    draftLayer.appendChild(path2);
    state.inkDraft.strokes = [stroke2];
    state.inkDraft.activePointerId = pointerEvent.pointerId;
    state.inkDraft.activePointerType = stroke2.pointer_type;
    state.inkDraft.activePath = path2;
...
function finalizeInkStroke(pointerEvent) {
  if (state.inkDraft.activePointerId !== pointerEvent.pointerId) return false;
...
export {
  activeArtifactKindForInk,
  appendInkPointToPath,
  beginInkStroke,
  buildInkPNGBase64,
  buildInkSVGMarkup,
  clearInkDraft,
