...
function setSyncKeyboardStateNow(sync) {
...
function toggleTTSSilentMode() {
  ...
  updateRuntimePreferences({ silent_mode: next }).then(() => {
...
["touchstart", "touchend", "mousedown", "keydown"].forEach(
  (evt) => document.body.addEventListener(evt, unlockAudioContext, { once: false })
);
function initRuntimeUi() {
...
function showVoiceCaptureNotice(message, x = null, y = null) {
  ...
  showStatus(text);
  const now = Date.now();
  if (text !== lastVoiceCaptureNoticeText || now - lastVoiceCaptureNoticeAt > 2e3) {
...
function persistRuntimeReloadContext(reason = "") {
  ...
  try {
    window.sessionStorage.setItem(RUNTIME_RELOAD_CONTEXT_STORAGE_KEY, JSON.stringify(context2));
  } catch (_) {
  }
}
...
function restoreRuntimeReloadContext() {
  const context2 = state.pendingRuntimeReloadContext;
  state.pendingRuntimeReloadContext = null;
  if (!context2 || typeof context2 !== "object") return;
  const edgeTop = document.getElementById("edge-top");
  if (edgeTop instanceof HTMLElement) {
    edgeTop.classList.toggle("edge-pinned", context2.edgeTopPinned === true);
  }
  const edgeRight = document.getElementById("edge-right");
  if (edgeRight instanceof HTMLElement) {
    edgeRight.classList.toggle("edge-pinned", context2.edgeRightPinned === true);
  }
  const chatHistory = document.getElementById("chat-history");
  if (chatHistory instanceof HTMLElement) {
    const top = Number(context2.chatScrollTop);
    chatHistory.scrollTop = Number.isFinite(top) ? top : 0;
  }
  const scrollX = Number(context2.windowScrollX);
  const scrollY = Number(context2.windowScrollY);
  ...
  if (String(context2.reason || "").trim().toLowerCase() === "deployment") {
...
function normalizeActiveSphere(raw) {
  return String(raw || "").trim().toLowerCase() === "work" ? "work" : "private";
}
function readPersistedActiveSphere() {
  try {
...
function persistActiveSpherePreference(sphere) {
  try {
...
function showDisclaimerModal() {
  ...
  return new Promise((resolve, reject) => {
    ...
    btn.addEventListener("click", () => {
      btn.disabled = true;
      acknowledgeDisclaimer(state.disclaimerVersion).then(() => {
...
function isRuntimeReloadWatcherScheduled() {
...
function isArtifactEditorActive() {
...
function isTextArtifactPaneActive() {
  ...
  return pane instanceof HTMLElement && pane.classList.contains("is-active") && window.getComputedStyle(pane).display !== "none";
}
...
function applyArtifactEditorText(text) {
  ...
  renderCanvas({
    event_id: getActiveTextEventId() || void 0,
    kind: "text_artifact",
...
function normalizeCompanionIdleSurface(raw) {
  return String(raw || "").trim().toLowerCase() === COMPANION_IDLE_SURFACES.BLACK ? COMPANION_IDLE_SURFACES.BLACK : COMPANION_IDLE_SURFACES.ROBOT;
}
...
function updateCompanionIdleSurface() {
  ...
  if (detailNode) {
    ...
    detailNode.textContent = runtimeDetail && runtimeState !== COMPANION_RUNTIME_STATES.IDLE ? runtimeDetail.replaceAll("_", " ") : copy.detail;
  }
}
...
export {
  ...
  activeProject,
  activeProjectKey,
  applyArtifactEditorText,
  applyCompanionState,
  applyIPhoneFrameCorners,
  applyRuntimePreferences,
  applyRuntimeReasoningEffortOptions,
  artifactEditorEl,
  canEnterArtifactEditModeFromTarget,
  clearKeyboardOpenState,
  closeDisclaimerModal,
  companionIdleSurfaceEl,
  companionStatusCopy,
  consumeRuntimeReloadContext,
  defaultReasoningEffortForAlias,
  ensureArtifactEditor,
  enterArtifactEditMode,
  exitArtifactEditMode,
  extractMathSegments,
  fetchRuntimeMeta,
  forceUiHardReload,
  getTTSAudioContext,
  hasVisibleCanvasArtifact,
  hubProject,
  initPanelMotionMode,
  initRuntimeUi,
  isArtifactEditorActive,
  isEditableTarget,
  isFocusedTextInput,
  isHubActive,
  isHubProject,
  isIPhoneStandalone,
  isMobileSilent,
  isRuntimeReloadWatcherScheduled,
  isSuppressedClick,
  isTemporaryProjectKind,
  isTextArtifactPaneActive,
  measureEditorCharWidth,
  mediaQueryMatches,
  microphoneUnavailableMessage,
  normalizeActiveSphere,
  normalizeCompanionIdleSurface,
  normalizeCompanionRuntimeState,
  normalizeProjectChatModelAlias,
  normalizeProjectChatModelReasoningEffort,
  normalizeReasoningEffortOptions,
  normalizeReasoningEffortOptionsByAlias,
  offsetFromLineAndColumn,
  parseCssPx,
  persistActiveSpherePreference,
  persistRuntimeReloadContext,
  placeArtifactEditorCaretFromPoint,
  pollRuntimeForRuntimeReload,
  readPersistedActiveSphere,
  reasoningEffortOptionsForAlias,
  resetCompanionState,
  restoreMathSegments,
  restoreRuntimeReloadContext,
  setSyncKeyboardStateNow,
  setTTSSilentMode,
  settleKeyboardAfterSubmit,
  shouldEnablePanelMotion,
  shouldShowCompanionIdleSurface,
  showDisclaimerModal,
  showStatus,
  showVoiceCaptureNotice,
...
//# sourceMappingURL=app-runtime-ui.js.map
