...
import { escapeHtml, sanitizeHtml as sanitizeHtml2 } from "./canvas-content.js";
import { resolveCanvasApprovalRequest } from "./canvas-actions.js";
...
const pdfRenderState = {
  ...
  resizeTimer: null,
  lastWidth: 0,
  renderTasks: /* @__PURE__ */ new Set(),
  textLayers: /* @__PURE__ */ new Set()
};
...
function surroundingTextForLine(lines, line) {
  ...
  return lines.slice(start, end).map((entry, index) => `${start + index + 1}: ${String(entry || "")}`).join("\n").trim();
}
...
function getDiffAnchorContext(node) {
  ...
  const resolvedLine = Number.isFinite(fileLine) && fileLine > 0 ? fileLine : Number.isFinite(diffLine) && diffLine > 0 ? diffLine : null;
...
function getLocationFromPoint(clientX, clientY) {
  const e = getEls();
...
function getLocationFromSelection() {
  const e = getEls();
...
function showLineHighlight(clientX, clientY) {
  clearLineHighlight();
  const e = getEls();
  if (!e.text) return;
  const range = textRangeFromPointInRoot(e.text, clientX, clientY);
  ...
  if (range && e.text.contains(range.startContainer)) {
    ...
    const estimate = estimateTextLineAtPoint(e.text, clientY);
    if (!estimate) return;
    const rootRect2 = e.text.getBoundingClientRect();
    rangeRect = {
      top: estimate.top,
      height: estimate.height,
      bottom: estimate.top + estimate.height,
      left: rootRect2.left,
      right: rootRect2.right
    };
  }
...
function getPreviousArtifactText() {
