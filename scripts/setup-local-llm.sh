...
find_llama_server() {
  if [ -n "${LLAMA_SERVER_BIN:-}" ] && command -v "$LLAMA_SERVER_BIN" >/dev/null 2>&1; then
  ...
  local devstral_bin="${HOME}/.local/llama.cpp/llama-server"
  if [ -x "$devstral_bin" ]; then
    printf '%s' "$devstral_bin"
    return 0
  fi
  return 1
}
...
SERVER_BIN="$(find_llama_server)" || {
  echo "llama.cpp server binary not found." >&2
